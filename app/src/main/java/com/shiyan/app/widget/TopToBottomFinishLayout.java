    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                LogUtils.eTag("下滑测试","moveY:" + moveY);
                // 满足此条件屏蔽SildingFinishLayout里面子类的touch事件
                if (Math.abs(moveY - downY) > mTouchSlop
                        && Math.abs((int) ev.getRawX() - downX) < mTouchSlop && moveY > downY) {
                    return true;
                }
                break;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                tempY = moveY;
                if (Math.abs(moveY - downY) > mTouchSlop
                        && Math.abs((int) event.getRawX() - downX) < mTouchSlop && moveY > downY) {
                    LogUtils.eTag("下滑测试","moveY:" + moveY);
                    isSilding = true;
                }
                ...
                if (moveY - downY >= 0 && isSilding) {
                    ...
                    LogUtils.eTag("下滑测试","moveY - downY:" + (moveY - downY));
                    if(onFinishListener != null){
                        onFinishListener.onProgress(moveY - downY);
                    }
                }
                break;
